...
|---|---|---|---|---|
| S14.5-T1 | Extract `libplayos-install` from the installer's step engine | `playos-refdistro` | not started | Behaviour-preserving: the standalone installer must still pass its on-device run |
...
| S14.5-T5 | On-device verification: shell-driven install, forced failure, standalone path | `playos-refdistro` | not started | Evidence in `playos-refdistro/docs/` |
