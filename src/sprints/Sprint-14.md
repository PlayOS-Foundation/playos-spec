...
| S14-T5 | Run the full 19-criterion MVP smoke test | `playos-refdistro` | in progress | Checklist `24dd9c8` + `scripts/mvp-smoke.sh` (fixed 2026-09-12: C3 read the wrong `comm` field, C8 used `ldd` which musl lacks, C2 documented the live-USB ESP path). On-device run 2026-09-12: **18/19 PASS**, report + raw evidence committed under `playos-refdistro/docs/`; criterion 19 (recovery without accelerated graphics) fails — see F3 |
| S14-T6 | Implement recovery mode | `playos-init`, `playos-shell`, `playos-refdistro` | done | Entry: cmdline (`playos.recovery`)/button hold (START+SELECT 2 s, Vol Up or Vol Down 5 s)/data-missing/compositor-failure; menu: reboot/shutdown/reset/rollback/logs (`1729329`, `e3c0091`); rollback via `RollbackSlot` IPC (shell no longer rewrites `boot.json`); late button watch non-blocking (no per-boot 4s delay/console prompt). **F3 closed 2026-09-13**: recovery rendering without an accelerated GPU is **partially** closed — kernel SimplEDRM (`FB/SYSFB/SIMPLEFB/DRM_SIMPLEDRM`), compositor software (pixman) path with a pre-flight DRM device probe, init forces `PLAYOS_RENDERER=pixman` for recovery and restarts a dead compositor in software mode; `playos.recovery` cmdline fixed (it was read before /proc existed). Verified in QEMU by `scripts/qemu-recovery-check.sh` (no GPU driver → recovery menu on screen). **Outstanding:** forcing the software renderer does not work for the UI on the Ally — the shell is a GL client and EGL fails over a pixman compositor (eglInitialize 0x3001), so recovery keeps the accelerated renderer and a GL-free recovery client is still needed |
| S14-T7 | Measure and document the performance baseline | `playos-refdistro` | in progress | Checklist `63a4d67` + collector script `59e3fd4`, extended 2026-09-12 with log-derived latencies and CPU sampling. On-device: shell→game first frame 1.00 s (pass), SYSTEM→overlay 4–5 ms (pass), game exit→shell 28 ms (pass), **boot→shell 7.16 s vs 5 s target (1.43×, gap P1)**, in-game FPS and direct scanout unverified (P2/P3), idle shell CPU 6 % of one core (P4). Report + evidence committed under `playos-refdistro/docs/`, gaps filed in `playos-spec/src/testing.md` |
