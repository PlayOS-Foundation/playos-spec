...
| S14-T5 | Run the full 19-criterion MVP smoke test | `playos-refdistro` | done | Checklist `24dd9c8` + `scripts/mvp-smoke.sh` (fixed 2026-09-12: C3 read the wrong `comm` field, C8 used `ldd` which musl lacks, C2 documented the live-USB ESP path). On-device run 2026-09-12: **18/19 PASS**, report + raw evidence committed under `playos-refdistro/docs/`; criterion 19 (recovery without accelerated graphics) fails — see F3 **Closed 2026-09-13: 19/19 PASS.** Criterion 19 (recovery usable without accelerated graphics) is met by the GL-free `playos-recovery` client and the compositor's software path — verified in QEMU (`scripts/qemu-recovery-check.sh`, incl. the `playos.noshell` hook) and on the Ally (automatic fallback, `registered as the shell role`, screenshot pair in `playos-refdistro/docs/evidence/`). See `docs/f3-recovery-software-rendering-2026-09-13.md`. |
| S14-T6 | Implement recovery mode | `playos-init`, `playos-shell`, `playos-refdistro` | done | Entry: cmdline (`playos.recovery`)/button hold (START+SELECT 2 s, Vol Up or Vol Down 5 s)/data-missing/compositor-failure; menu: reboot/shutdown/reset/rollback/logs; rollback via `RollbackSlot` IPC; button watch non-blocking. **F3 closed 2026-09-13** with a GL-free recovery UI: `playos-recovery` (wl_shm + software text) is started by init when the GL shell cannot run in recovery, so the menu still appears with no accelerated graphics; kernel SimplEDRM + the compositor's software path provide the display. Verified in QEMU (`scripts/qemu-recovery-check.sh`, incl. the `playos.noshell` hook) and the menu itself on the Ally |
| S14-T7 | Measure and document the performance baseline | `playos-refdistro` | done | Checklist `63a4d67` + collector script `59e3fd4`, extended 2026-09-12 with log-derived latencies and CPU sampling. On-device: shell→game first frame 1.00 s (pass), SYSTEM→overlay 4–5 ms (pass), game exit→shell 28 ms (pass), **boot→shell 7.16 s vs 5 s target (1.43×, gap P1)**, in-game FPS and direct scanout unverified (P2/P3), idle shell CPU 6 % of one core (P4). Report + evidence committed under `playos-refdistro/docs/`, gaps filed in `playos-spec/src/testing.md` **Closed 2026-09-13.** P2 done: the compositor logs per-role frame rate, so in-game FPS is measured rather than assumed (56-120 fps on the Ally). P3 done: presentation events show **100% direct scanout** (1684 zero-copy / 0 copied during a 30 s game run). P4 done: the shell is damage-driven — idle 55.5 → 8.0 fps and CPU 7.4% → 2.6% of one core, with the suspend gate confirmed in-game (0 shell commits/s while a game is foreground). P1 improved 7.66 → 6.49 s cold boot to shell; the remaining ~1.5 s is documented with its three structural causes in the perf report's follow-up sections.  **P1 resolved 2026-09-20**: installed boot 10.55 s → **3.28 s** to ShellReady (under the 5 s target) by having each image declare its boot medium on its compiled-in command line; the live path is 5.63 s (0.63 s over, device-bound on the dock USB hub bring-up). See `docs/perf-baseline-report-2026-09-12.md`. |
...
| S14-T9 | Enforce production image hygiene and signed artifacts | `playos-refdistro` | done | Production defconfig + `production-build.yml` + release pipeline (`acb7fe8`). Final run 2026-09-13: prod image hygiene verified (no `/bin/sh`, busybox, dropbear/sshd), EFI kernel signed and `sbverify` OK, prod `update.playosb` built and its HMAC + payload hash independently verified, SDK headers tarball compiles a minimal game with `-Werror`; and the **A/B update was applied on the Ally** — slot B written byte-exactly, `boot.json` flipped to b/0.3.0, system booted slot B; and the **rollback** half via the recovery menu (`RollbackSlot` IPC → slot A, the left slot quarantined `bad`). Evidence: `docs/t9-signed-artifacts-2026-09-13.md` |
| S14-T10 | Installer as a PlayOS app with seamless handoff | `playos-shell`, `playos-init`, `playos-refdistro` | done | Shell owns the front-end (`SCREEN_INSTALLER`: disk list from `/sys/block`, hold-A confirm, sends `StartInstaller` with `target_disk`); init forwards `PLAYOS_INSTALL_TARGET` and performs a **seamless handoff** — console suppressed, only the UI clients make way, compositor and `/data` stay up, only the target's ESP is released, failed install returns to the shell (init `48dc60b`, `e3b9e31`, `d70dc8f`); installer restyled to the shell's Silkscreen/navy look and releases the target's mounts itself (refdistro `d67d2e0`, `a634acc`). **Verified on the Ally: all 8 install steps ok, boots the installed system** (`boot.json` slot a good, boot_count 1, 11 games seeded) |
...
Implement a minimal recovery UI (text or simple Raylib on SimpleDRM/framebuffer). Recovery entry points are: boot count exceeds the A/B limit with both slots bad, a button hold at boot, and `playos-init` entering recovery after repeated compositor failure. The accepted holds (`playos_recovery_button_held()`: polled once at startup and from the non-blocking late watch) are **START+SELECT together for 2 s**, **Volume Up for 5 s** and **Volume Down for 5 s** — as implemented by `trigger_gamepad`/`trigger_volume_up`/`trigger_volume_down` in `recovery.c`. The menu offers: view system logs (`/data/log/`), factory reset (Sprint 10 logic), rollback to the previous system slot when available, shutdown, and reboot. Recovery must work without AMDGPU.
