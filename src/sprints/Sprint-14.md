...
| S14-T10 | Installer as a PlayOS app with seamless handoff | `playos-shell`, `playos-init`, `playos-refdistro` | done | Console-free handoff: init suppresses kernel console + blanks VT (`49fd28e`); installer fullscreen splash covers transition (`1a7ace4`). Front-end is now the shell's own **Installer Screen** (`SCREEN_INSTALLER`): disk list from `/sys/block`, destructive hold-A confirm, sends `StartInstaller` with `target_disk` (shell `d084b6f`); init forwards it as `PLAYOS_INSTALL_TARGET` (`87ae3bb`) and the installer skips its picker (`302bb60`) |
